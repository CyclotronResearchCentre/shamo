...
"""def test_add_anisotropy_in_tissue_from_elements_scalar():
    with TemporaryDirectory() as parent_path:
        ...
        gmsh.finalize()
        model.add_anisotropy_in_tissue_from_elements(element_tags,
                                                     element_values,
...
def test_add_anisotropy_in_tissue_from_elements_vector():
    with TemporaryDirectory() as parent_path:
        shutil.copytree(str(Path("tests/data/only_geometry")),
                        str(Path(parent_path) / "only_geometry"))
        model = FEModel.load(str(Path(parent_path)
...
def test_add_anisotropy_in_tissue_from_elements_tensor():
    with TemporaryDirectory() as parent_path:
        shutil.copytree(str(Path("tests/data/only_geometry")),
                        str(Path(parent_path) / "only_geometry"))
        model = FEModel.load(str(Path(parent_path)
...
def test_add_anisotropy_in_tissue_from_array_scalar():
    with TemporaryDirectory() as parent_path:
        shutil.copytree(str(Path("tests/data/only_geometry")),
                        str(Path(parent_path) / "only_geometry"))
        model = FEModel.load(str(Path(parent_path)
        ...
        model.add_anisotropy_in_tissue_from_array(field, affine, in_tissue,
                                                  fill_value=0)
        assert("b" in model.anisotropy)
        assert(model.anisotropy["b"].anisotropy_type == Anisotropy.SCALAR)


def test_add_anisotropy_in_tissue_from_array_vector():
    with TemporaryDirectory() as parent_path:
        shutil.copytree(str(Path("tests/data/only_geometry")),
                        str(Path(parent_path) / "only_geometry"))
        model = FEModel.load(str(Path(parent_path)
...
def test_add_anisotropy_in_tissue_from_array_tensor():
    with TemporaryDirectory() as parent_path:
        shutil.copytree(str(Path("tests/data/only_geometry")),
                        str(Path(parent_path) / "only_geometry"))
        model = FEModel.load(str(Path(parent_path)
