...
class EEGSimulationProblem(EEGForwardProblem):
    ...
    TEMPLATE_PATH = resource_filename("shamo", str(
        Path("problems/forward/eeg/eeg_simulation_problem.pro")))
    ...
    def check_settings(self, model, **kwargs):
        ...
        for source in self.sources:
            if not model.source_exists(source):
                raise ValueError(("Source located at '({:.3f}, {:.3f}, "
                                  "{:.3f})' not defined in the "
                                  "model.").format(*source.coordinates))
    ...
    def solve(self, name, parent_path, model, **kwargs):
        ...
        with TemporaryDirectory(dir=scratch_path) as temporary_path:
            ...
            values = np.loadtxt(str(Path(temporary_path) / node_file_name),
                                skiprows=1)
            sensor_index = np.where(
                values[:, 0] == model.sensors[self.reference].node)
            ...
            for name, sensor in sensors.items():
                ...
                recordings[name] = float(values[sensor_index, 1]
                                         - reference_value)
    ...
    def _generate_problem_file(self, path, model):
        ...
        with TemplateFile(self.TEMPLATE_PATH, path) as template_file:
            # Add the regions
            tissue_regions = {name: tissue.volume_group
                              for name, tissue in model.tissues.items()}
            template_file.replace_with_dict("tissues", "region",
                                            tissue_regions,
            ...
            template_file.replace_with_dict("tissues", "sigma",
                                            electrical_conductivity,
                                            key_value_separator="] = ",
                                            prefix="sigma[", suffix=";",
                                            separator="\n    ")
            ...
            for i_source, source in enumerate(self.sources):
                ...
                for i_axis, value in enumerate(source.values):
                    if value != 0:
                        ...
                        source_regions[name] = \
                            model_source.point_groups[i_axis][0]
                        source_names.append(name)
                        source_currents[name] = value / model_source.length
                        # Second point
                        name = "Source_{}_{}_1".format(i_source, i_axis)
                        source_regions[name] = \
                            model_source.point_groups[i_axis][1]
                        source_names.append(name)
                        source_currents[name] = -value / model_source.length
            template_file.replace_with_dict("sources", "region",
                                            source_regions,
                                            key_value_separator=" = Region[{",
                                            suffix="}];", separator="\n    ")
            template_file.replace_with_list("sources", "name", source_names,
                                            separator=", ")
            template_file.replace_with_dict("sources", "current",
                                            source_currents,
                                            key_value_separator="] = ",
        ...
        with open(path, "r") as pos_file:
            for i_line, line in enumerate(pos_file):
                if i_line > 0 and line != "};\n" and line != "":
                    ...
                    values = [float(match) - reference_potential
                              for match in re.findall(r"[\-\.\d]{2,}",
                                                      parts[1])]
                    new_values = ",".join([str(value[0][0])
                                           for value in values])
